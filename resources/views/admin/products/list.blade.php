                    ...
                    <tbody>
                        @forelse ($products as $product)
                            <tr class="product-row">
                                <td>
                                    <button class="btn btn-sm btn-outline-primary toggle-lots-btn"
                                        data-id="{{ $product->id }}"
                                        data-url="{{ route('admin.products.fetchLots', $product->id) }}">
                ...
                if (lotsContainerRow.is(':visible')) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            icon.removeClass('bi-arrow-clockwise anim-spin').addClass(
                                'bi-dash-lg');

                            const imageUrlTemplate =
                                '{{ route('admin.products.qrImage', ['lot' => 0]) }}';
                            const downloadUrlTemplate =
                                '{{ route('admin.products.qrDownload', ['lot' => 0]) }}';

                            let content = '<h6 class="mb-2">Associated Lots:</h6>';
                            if (data.length > 0) {
                                content += '<ul class="list-group">';
                                $.each(data, function(index, lot) {
                                    const imageUrl = imageUrlTemplate.replace('/0/',
                                        `/${lot.id}/`);
                                    ...
                                    content += `<li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <button class="btn btn-sm btn-outline-secondary show-qr-btn"
                                                    ...
                                                    data-qr-image-url="${imageUrl}"
                                                    data-qr-download-url="${downloadUrl}">
                        ...
                        error: function() {
                            icon.removeClass('bi-arrow-clockwise anim-spin').addClass(
                                'bi-plus-lg');
                            lotsDiv.html(
                                '<p class="text-center text-danger">Failed to load lots.</p>'
                            );
                        }
                    });
                }
